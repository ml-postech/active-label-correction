...
def get_argparser():
    ...
    # Datset Options
    parser.add_argument("--data_root", type=str, default='/hdd/hdd4/khy/Revisiting/deeplab/datasets/cityscapes',
...
def get_dataset(opts):
    ...
    if 'voc' in opts.dataset:
        ...
        train_dst = VOCSegmentation(root=opts.data_root, year=opts.year,
                                    ...
                                    transform=train_transform, mask_dir=opts.mask_dir)
        val_dst = VOCSegmentation(root=opts.data_root, year=opts.year,
                                  image_set='val', download=False, 
                                  transform=val_transform, mask_dir=None)

    if 'city' in opts.dataset:
        train_transform = et.ExtCompose([
            # et.ExtResize( 512 ),
        ...
        train_dst = Cityscapes(root=opts.data_root, split='train', 
                               transform=train_transform, mask_dir=opts.mask_dir)
...
def main():
    ...
    opts.save = 'latest_%s_%s_%s.pth'%(opts.model, opts.dataset, opts.random_seed)
    if 'voc' in opts.dataset.lower():
        opts.num_classes = 21
    ...
    # Setup dataloader
    if 'voc' in opts.dataset and not opts.crop_val:
    ...
    elif opts.loss_type == 'gce':
        print("gce")
    elif opts.loss_type == 'pgce':
        print("pgce")
    ...
    denorm = utils.Denormalize(mean=opts.mean, std=opts.std)  # denormalization for ori images
